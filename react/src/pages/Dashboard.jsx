...
const Dashboard = () => {
    ...
    const { data: complaintsByCity, loading: cityLoading, error: cityError, refetch: refetchCity } = useApi(dashboardAPI.getComplaintsByCity);
    const { data: complaintsByCategory, loading: categoryLoading, error: categoryError, refetch: refetchCategory } = useApi(dashboardAPI.getComplaintsByCategory);
    const { data: departmentRatings, loading: ratingsLoading, error: ratingsError, refetch: refetchRatings } = useApi(dashboardAPI.getDepartmentRatings);
    const { data: monthlyTrends, loading: trendsLoading, error: trendsError, refetch: refetchTrends } = useApi(dashboardAPI.getMonthlyTrends);
    const { data: recentComplaints, loading: recentLoading, error: recentError, refetch: refetchRecent } = useApi(() => dashboardAPI.getRecentComplaints(10));
    ...
    const statCards = [
        ...
        },
        {
            ...
            value: stats ? `${Math.round((stats.resolved_complaints / stats.total_complaints) * 100)}%` : '0%',
            icon: Activity,
    ...
    const renderOverviewTab = () => (
        ...
        <div className="space-y-8">
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <div className={`flex items-center space-x-1 text-sm font-medium ${card.changeType === 'increase' ? 'text-green-600' : 'text-red-600'
                                        }`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                       ...
                       <p>Something went wrong Try again</p>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={complaintsByCity}>
                                ...
                                />
                                <Bar dataKey="complaint_count" fill="url(#colorGradient1)" radius={[4, 4, 0, 0]} />
                                <Bar dataKey="resolved_count" fill="url(#colorGradient2)" radius={[4, 4, 0, 0]} />
                                <defs>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-semibold text-gray-900">Complaints by Category</h3>
                        <button
                ...
                {recentLoading ? (
                    ...
                    <div className="space-y-4">
                        {recentComplaints?.map((complaint) => (
                            <div key={complaint.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-gray-100">
                                ...
                                <div className="flex items-center space-x-3">
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${complaint.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${complaint.priority >= 4 ? 'bg-red-100 text-red-800' :
                                            complaint.priority === 3 ? 'bg-yellow-100 text-yellow-800' :
                                                'bg-green-100 text-green-800'
                                        }`}>
                                        Priority {complaint.priority === 1 ? 'Low' : complaint.priority === 2 ? "Medium" : complaint.priority === 3 ? "High" : "Critical"}
                                    </span>
                                </div>
                            </div>
                        ))}
                                ...
                                <div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-gray-100">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center space-x-4 mt-2">
                                            <span className="text-sm text-gray-500">{category.complaint_count} complaints</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-3">
                    ...
                    {ratingsLoading ? (
                        ...
                        <div className="space-y-4">
                            {departmentRatings?.map((dept, index) => (
                                <div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-purple-50 rounded-xl border border-gray-100">
                                    <div className="flex-1">
                                        <h4 className="font-medium text-gray-900">{dept.department_name}</h4>
                                        <div className="text-sm text-gray-500">{dept.total_feedback} reviews</div>
                                    </div>
                                    <div className="flex items-center space-x-2">
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white">
                    ...
                    <div className="text-3xl font-bold mb-2">{stats?.resolved_complaints || 0}</div>
                ...
                <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white">
                    ...
                    <div className="text-3xl font-bold mb-2">{stats?.average_rating || '0.0'}/5</div>
                ...
                {ratingsLoading ? (
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                                ...
                                data={departmentRatings.map(item => (
                                    ...
                                    />
                                    <Bar dataKey="avg_rating" fill="url(#colorGradient1)" radius={[4, 4, 0, 0]} />
