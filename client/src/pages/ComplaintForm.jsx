...
const ComplaintForm = () => {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (formData.citizen_name.trim() < 5)
        newErrors.citizen_name = "Name must be atleast 5 characters";

      if (!formData.citizen_phone.trim())
        newErrors.citizen_phone = "Phone is required";

      if (!/^[0-9]+$/.test(formData.citizen_phone))
        newErrors.citizen_phone = "Phone must contain numbers only";
    ...
    if (step === 2) {
      ...
      if (formData.address.trim() < 5)
        newErrors.address = "address must contain 5 characters";
      if (formData.landmark.trim() < 5)
        newErrors.landmark = "landmark must contain 5 characters";
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
              errors.citizen_name ? "border-red-500" : "border-gray-300"
            }`}
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors ${
              errors.citizen_email ? "border-red-500" : "border-gray-300"
            }`}
                ...
                <div
                  className={`flex items-center space-x-3 p-3 rounded-lg border-2 transition-all duration-200 ${
                    formData.priority === option.value
                      ? `${option.borderColor || "border-blue-500"} ${
                          option.bg
                        }`
                  ...
                  <span
                    className={`font-medium ${
                      option.textColor || option.color || "text-gray-700"
                    }`}>
                    {option.label}
                  </span>
                </div>
      ...
      {submitSuccess ? (
        <>
          ...
          <p className="text-lg text-gray-600 mb-2">
            Your complaint has been registered and assigned ID:{" "}
            <strong>{complaintResult?.id}</strong>
          </p>
          ...
          <div className="bg-blue-50 rounded-xl p-6 mb-8">
            ...
            <div className="space-y-3 text-left">
              ...
              <div className="flex items-center space-x-3">
                ...
                <span className="text-blue-800">
                  A worker will be assigned to resolve the issue
                </span>
              </div>
              <div className="flex items-center space-x-3">
